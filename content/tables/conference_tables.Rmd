...
```{r conferencetables}
league_dat <- data.frame(read.csv('round6.csv'))
...
form <- list(c(0,1,0,1,1,0), # Sam
             c(1,1,1,1,1,1), # Ben
             c(1,0,0,1,1,1), # Rhu
             c(0,0,1,0,0,0), # Cheese
             c(0,1,1,0,0,1), # Steggles
             c(1,0,0,0,0,0), # Jamie
             c(1,0,0,1,0,1), # Dan
             c(1,1,0,1,1,1), # Alex
             c(0,0,1,1,1,0), # Jack
...
ggplot(league_dat, 
  ...
  geom_vline(xintercept=mean(league_dat$Points.Scored),
             linetype="dashed", color = "black") +
  annotate("text", 
           x = mean(league_dat$Points.Scored)-35, 
           y = mean(league_dat$Points.Conceded)-25, 
           label = ("Missed Chances"), 
           fontface = 2, alpha = 0.5) +
  annotate("text", 
           x = mean(league_dat$Points.Scored)-35, 
           y = mean(league_dat$Points.Conceded)+35, 
...
ctable <- league_dat %>%
  # decide on the ranking order of priority
  arrange( -league_dat$Points, 
           -league_dat$Pts.Diff,
           league_dat$Team.Name) %>%
  # add in the rank collumn
  mutate(Rank = 1:n())

  ctable$PosChng <- as.numeric(unlist(ctable$Previous.Position)) - ctable$Rank
  
  ctable <- select(ctable, -Previous.Position) %>%
  # create as gt
    gt() %>%
  # create win-loss plot in place of the form variable and move to after the loss collumn
  gt_plt_winloss(Form,
  ...
  tab_style(
    ...
    locations = cells_body(rows = Rank > 8)) %>%
  # as
  tab_style(
    style = list(
      cell_fill(color = "green", 
                alpha = 0.1),
      cell_text(color = "black")),
    locations = cells_body(rows = Rank < 9)) %>%
  # add in footnotes
  tab_footnote(
    footnote = "1 win = 4 points",
    locations = cells_column_labels(columns = Wins)) %>%
  ...
  gt_merge_stack(col1 = Team.Name, 
    ...
    cols_move(
      columns = PosChng,
      after = Rank) %>%
    gtsave_extra(., "combitab.png", path = NULL, ..., zoom = 2, expand = 5)
    #gtsave("combinedtable.png")
    
# call the table
ctable
```
```{r records}
# muck about with form
for (x in 1:12) {
a <- data.frame( # convert resultant output to a dataframe called a
  unclass( # remove the class
    rle( # look for consecutive sequences
      unlist(form[x]) # remove list classing from the form variable
      )
    )
  ) 

win_streak[x] <- a[a$values == "1", ] %>%
  max(.$lengths)

loss_streak[x] <- a[a$values == "0", ] %>%
  max(.$lengths)
}
```
